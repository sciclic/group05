# this script is basically the same as the clean.R script, but modifies a few things needed for the dashboardEDA.R script
# save a new object to use for that script instead

"This script loads in a data file and cleans the data file for the dashboard.
...
main <- function(filepath){
  ...
  ## Load the csv
  survey_raw <- read_csv(here::here("data", "survey_raw.csv"))
  ...
  survey_data <- 
    survey_data %>%
    select(c("q1", # which degree are you studying
             "q3", # which was the most important reason you decided to get enrolled in your PhD
  ...
  survey_data <-
    survey_data %>%
    dplyr::rename(degree = q1,
           most_important_reason_to_enroll_PhD = q3,
  ...
  survey_data <-
    survey_data %>%
    dplyr::select(c('age', 'gender', 'degree', 
             'most_important_reason_to_enroll_PhD',
  ...
  print("Data cleaning for dash is complete!")
}
